...
function view_item($get_value, $location_id, $category_id) {
	...
	return '<table>
			...
			<tr>
				<td></td>
				<td> <a href="dz_6_2.php">Свернуть</a></td>
			</tr>
	</table>';

}

// Функция вывода строки объявления
function print_item ($id, $item) {
	return '<p>' . $item['date'] .' | ' . '<a href="dz_6_2.php?view=' . $id . '">' . $item['title'] . '</a>' .' | ' . number_format($item['price'], 2, '.', '') . ' руб.' . ' | ' . $item['seller_name'] .' | ' . '<a href="dz_6_2.php?delete=' . $id . '">Удалить</a>' . "</p>\n\r";
...
if (isset($_SESSION['bd'])){
	foreach ($_SESSION['bd'] as $id => $item){
		if (isset($_SESSION['view']) && $_SESSION['view']==$id ) { // Если есть команда на просмотр объявления
				$get_value=$_SESSION['view'];
					echo print_item ($id, $item);
					echo view_item($get_value, $location_id, $category_id);
					unset ($_SESSION['view']);
			}
			else { // Вывод обычной строки объявления
				echo print_item ($id, $item);
			}
	}
}