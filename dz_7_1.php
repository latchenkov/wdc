...
// Переносим данные из $_COOKIE в рабочий массив	
if (isset($_COOKIE['ads_db'])){
	$ads_db = unserialize($_COOKIE['ads_db']);
}
// Переносим данные из $_POST в $_COOKIE
...
// Обработка команды на на удаление
...
if (isset($ads_db['db'])){
		...
		echo '<p>' . $item['date'] .' | ' . '<a href="dz_7_1.php?show=' . $id . '">' . $item['title'] . '</a>' .' | ' . number_format($item['price'], 2, '.', '') . ' руб.' . ' | ' . $item['seller_name'] .' | ' . '<a href="dz_7_1.php?delete=' . $id . '">Удалить</a>' . "</p>\n\r";
	}
}